...
6. **Pressure-testing against an external critique — and discarding most of it.** I ran the page through a second AI for a cold-traffic/CRO teardown. It hallucinated a review of the *UK* product (wrong price, wrong teacher, wrong exam boards, "add a sticky CTA" we already had). Rather than action a generic checklist, I validated each point against the actual page and kept only the one idea that fit the strategy — the audience toggle — discarding the rest. Knowing which AI output to *reject* is as much the job as knowing what to use.
...
- **A parent/student toggle in the hero — and the discipline to stop it there.** Ad traffic for a tutoring product lands two different people: the *parent* (who pays and wants reassurance) and the *student* (who uses it and wants relief from feeling stupid). A hero toggle lets each self-select the framing that speaks to them — "the math help that finally sticks" + structure-and-results for the parent, "math that finally makes sense" + confidence for the student. The hero copy, the form heading, the helper text, and the grade-picker's matching beat all re-voice together so the student experience is coherent, not half-switched. **But it stops at the point money changes hands.** A 14-year-old can't enter a credit card, so re-voicing the *checkout* for a student would be optimising conversion for someone who can't convert — the student's job in this funnel is to get excited and pull in the parent, not to pay. So checkout and the billing copy stay parent/neutral by design. Knowing *where to stop* a personalisation is the real decision here: enough to match the message, not so much that I fork a funnel for a non-buyer.
- **The toggle also shows judgment about AI itself.** It came out of pressure-testing the page through a second AI, which hallucinated a review of the *UK* product (wrong price, wrong teacher, UK exam boards, "add a sticky CTA" we already had). Rather than action a generic checklist, I validated each point against the live page and kept exactly one idea — the audience toggle — because it was the only one that fit the strategy. Knowing which AI output to *reject* is as much the skill as knowing what to use.
- **A "what happens next" timeline on the success screen.** The highest-anxiety moment in a no-human funnel is the second after payment — "did it work, what now?" A 3-step confirmation (check email → first live class this week → only $7 today, $149 after 7 days) removes that anxiety, sets the attendance expectation that drives retention, and restates the billing terms one final time at the point they matter most.
